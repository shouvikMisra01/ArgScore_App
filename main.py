...
app = FastAPI(title="ArgScore MVP v0.3")
...
DOMAIN_TAXONOMY = [
    "physics", "engineering", "computer_science", "medicine", "biology",
    "economics", "political_science", "sociology", "law", "public_policy",
    "ethics", "philosophy", "business_strategy", "history", "journalism",
    "general",
]
...
def validate_weights(weights: Dict[str, float], bounds: Dict[str, List[float]]) -> None:
    """Clamp weights to bounds, then re-normalize to sum=1.0."""
    did_clamp = False
    for d in HARD_DIMENSIONS:
        if d not in weights:
            weights[d] = bounds[d][0]
            did_clamp = True
        w = float(weights[d])
        lo, hi = bounds[d]
        if w < lo:
    ...
    if did_clamp or abs(s - 1.0) > 1e-6:
        for d in HARD_DIMENSIONS:
...
def validate_dimension_scores(scores: Dict[str, float]) -> None:
    for d in HARD_DIMENSIONS:
        if d not in scores:
            scores[d] = 0.5  # Default instead of crash
        v = float(scores[d])
        scores[d] = clamp(v, 0.0, 1.0)
...
- If claim is not numeric: numeric MUST be null.
- checked_value can be null if unverifiable.
...
DOMAIN PACKET:
{json.dumps(domain_packet, indent=2, ensure_ascii=False)}
...
def chat_json(system_instruction: str, user_message: str) -> Dict:
    """Call OpenAI with JSON mode. Gemini version commented out below."""
    try:
...
# --- Gemini chat_json (Commented Out) ---
# def chat_json(system_instruction: str, user_message: str) -> Dict:
#     try:
...
async def analyze_argument(req: ArgumentRequest):
    ...
    gate = gate_res.get("gate")
    if gate not in ("PASS", "BLOCK"):
    ...
    bounds = domain_packet.get("allowed_weight_bounds", {})
    if not isinstance(bounds, dict) or len(bounds) == 0:
        # Fallback to defaults if AI didn't produce valid bounds
        bounds = DEFAULT_WEIGHT_BOUNDS
        domain_packet["allowed_weight_bounds"] = bounds

    validate_weight_bounds(bounds)

    # ── STAGE 3: Soft Rubric + Scoring ──
    ...
    if not isinstance(weights, dict):
        raise HTTPException(status_code=500, detail="soft_rubric.weights missing or invalid")
    if not isinstance(dim_scores, dict):
        raise HTTPException(status_code=500, detail="dimension_scores missing or invalid")
